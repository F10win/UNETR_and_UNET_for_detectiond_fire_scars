...
# === НАСТРОЙКИ ===
BASE_DIR = Path(r"E:\миигаик\1_Магистратура\Диплом\проект\UNETR_Burn\Ilusha\dataset\13_channels")
...
PRED_SAVE_DIR = Path(r"F:\Dataset\Basic UNET prediction\11 channels")
PRED_SAVE_DIR.mkdir(parents=True, exist_ok=True)

CHANNELS = 11  # ✅ 11 каналов (B2-B9, B8A, B11, B12)
IMG_SIZE = (512, 512)
BATCH_SIZE = 1
MAX_EPOCHS = 100
PATIENCE = 10
...
def setup_logging():
    ...
    # 📁 Путь для логов
    log_dir = Path(r"E:\миигаик\1_Магистратура\Диплом\проект\UNETR_Burn\UNETR_Burn\UNETR_Burn\logs\NewTrain\BasicUNET\11channels")
    log_dir.mkdir(parents=True, exist_ok=True)
    
    log_file = log_dir / f"training_basicunet_11ch_{timestamp}.log"
    metrics_csv = log_dir / f"metrics_basicunet_11ch_{timestamp}.csv"
...
def find_source_image(patch_folder):
    """
    Ищет исходное изображение в папке патча.
    Проверяет оба варианта: after_11ch.tif и after_13ch.tif
    """
    image_path_11ch = patch_folder / "after_11ch.tif"
    image_path_13ch = patch_folder / "after_13ch.tif"
    
    if image_path_11ch.exists():
        return image_path_11ch
    elif image_path_13ch.exists():
        return image_path_13ch
    else:
        return None
...
def main():
    ...
    logger.info("="*70)
    logger.info("🔷 Обучение Basic UNet на 11 каналах (80/20 сплит)")
    ...
    train_loader, val_loader = get_dataloaders(
        base_path=BASE_DIR, channels='11', batch_size=BATCH_SIZE,
    ...
    epochs_no_improve = 0
    best_model_path = CHECKPOINT_DIR / "basicunet_11ch_best.pth"  # ✅ Имя для 11 каналов
    start_epoch = 0
                    ...
                    with torch.amp.autocast('cuda'):
                        for batch_idx, (images, masks) in enumerate(val_loader):
                            ...
                            if batch_idx < len(val_patches):
                                ...
                                # 🔍 Ищем исходный файл (поддержка обоих имён)
                                source_image_path = find_source_image(source_patch)
                                ...
                                if source_image_path and source_image_path.exists():
                                    ...
                                    logger.info(f"  💾 Сохранено: pred_{saved_count:03d}.tif (GeoTIFF) из {source_image_path.name}")
                                else:
                                    logger.warning(f"  ⚠️  Исходный файл не найден в {source_patch}")
                            else:
    ...
    logger.info("\n" + "="*70)
    logger.info("🎉 ОБУЧЕНИЕ BASIC UNET (11 КАНАЛОВ) ЗАВЕРШЕНО!")
