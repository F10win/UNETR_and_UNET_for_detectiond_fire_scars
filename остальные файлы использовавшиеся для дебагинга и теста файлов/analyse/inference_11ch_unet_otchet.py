...
# -*- coding: utf-8 -*-
"""
Тестирование модели BasicUNet на новых снимках (11 каналов)
Адаптировано для Sentinel-2 с 11 спектральными каналами
"""
...
print("="*80)
print("🔥 ИНФЕРЕНЦИЯ МОДЕЛИ BasicUNet (11 КАНАЛОВ) НА НОВЫХ СНИМКАХ")
print("="*80)

# === НАСТРОЙКИ ===
# 📁 Путь к чекпоинту BasicUNet 11ch
MODEL_PATH = Path(r"E:\миигаик\1_Магистратура\Диплом\проект\UNETR_Burn\UNETR_Burn\UNETR_Burn\checkpoints\NewTrain\basicunet_11ch_best.pth")
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

INPUT_IMAGE = Path(r"E:\миигаик\1_Магистратура\Диплом\проект\UNETR_Burn\Ilusha\raw_data\region_15\20210807T020701_20210807T020656_T54VXQ_13chan_after.tif")
...
print(f"📡 Каналы: 11 (B2, B3, B4, B5, B6, B7, B8, B8A, B11, B12)")
...
if image.shape[0] != 11:
    print(f"⚠️  Предупреждение: Ожидается 11 каналов, найдено {image.shape[0]}.")
    print("   Используемые каналы: B2, B3, B4, B5, B6, B7, B8, B8A, B11, B12")
...
# ✅ Обновлённый путь для BasicUNet результатов
output_dir = Path(r"E:\миигаик\1_Магистратура\Диплом\проект\UNETR_Burn\UNETR_Burn\UNETR_Burn\analysis\inference_results_basicunet_11ch") / f"result_{timestamp}"
...
# Для 11 каналов показываем композит: B12(SWIR), B8(NIR), B4(Red) - индексы [10, 7, 3]
ax1.imshow(np.transpose(image_norm[[10, 7, 3], :, :], (1, 2, 0)))
ax1.set_title('Псевдоцвета (B12, B8, B4)', fontsize=14, fontweight='bold')
ax1.axis('off')
...
ax4.imshow(np.transpose(image_norm[[10, 7, 3], :, :], (1, 2, 0)))
...
ax6.axis('off')
# ✅ Обновлённый текст статистики для BasicUNet
stats_text = f"""
📊 СТАТИСТИКА АНАЛИЗА (BasicUNet 11 КАНАЛОВ)
...
plt.suptitle(f'🔥 Анализ гарей (BasicUNet 11ch): {INPUT_IMAGE.name}', fontsize=18, fontweight='bold', y=0.995)
plt.tight_layout()
...
if not df.empty:
    ...
    with open(txt_path, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write(f"ОТЧЁТ: АНАЛИЗ ЛЕСНЫХ ГАРЕЙ (BasicUNet 11 КАНАЛОВ)\n")  # ✅ Обновлён заголовок
        f.write("="*80 + "\n\n")
        ...
        f.write(f"📡 Использовано каналов: 11 (B2-B9, B8A, B11, B12)\n")
