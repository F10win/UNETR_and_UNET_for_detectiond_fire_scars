...
# -*- coding: utf-8 -*-
"""
Тестирование модели UNETR на новых снимках
ИСПРАВЛЕННАЯ ВЕРСИЯ
"""
...
print("="*80)
print("🔥 ИНФЕРЕНЦИЯ МОДЕЛИ UNETR НА НОВЫХ СНИМКАХ")
print("="*80)

# === НАСТРОЙКИ ===
MODEL_PATH = Path(r"E:\миигаик\1_Магистратура\Диплом\проект\UNETR_Burn\UNETR_Burn\UNETR_Burn\checkpoints\NewTrain\unetr_3ch_best.pth")
...
binary_mask = (pred_map > THRESHOLD).astype(np.uint8)  # ← Важно: uint8!
...
pixel_size_m = abs(src_transform.a)  # ← Добавлено: определение размера пикселя
...
output_dir = Path(r"E:\миигаик\1_Магистратура\Диплом\проект\UNETR_Burn\UNETR_Burn\UNETR_Burn\analysis\inference_results") / f"result_{timestamp}"
...
ax1.imshow(np.transpose(image_norm[:3], (1, 2, 0)))
...
ax4.imshow(np.transpose(image_norm[:3], (1, 2, 0)))
...
plt.suptitle(f'🔥 Анализ гарей: {INPUT_IMAGE.name}', fontsize=18, fontweight='bold', y=0.995)
plt.tight_layout()
...
if not df.empty:
    ...
    with open(txt_path, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write("ОТЧЁТ: АНАЛИЗ ЛЕСНЫХ ГАРЕЙ (UNETR)\n")
        f.write("="*80 + "\n\n")
