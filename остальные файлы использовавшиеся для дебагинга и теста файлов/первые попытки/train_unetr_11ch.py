...
log_file = start_logging(model_name="11ch")  # или "11ch" для другой модели
# =======================================
...
def train_fold(fold_num, train_indices, val_indices):
    ...
    epochs_no_improve = 0
    best_model_path = CHECKPOINT_DIR / f"unetr_11ch_fold{fold_num+1}.pth"
    
    print(f"🚀 Начало обучения на {DEVICE} (FP32, без AMP)\n")
    
    # ← ЗАГРУЗКА ЧЕКПОИНТА (если есть)
    checkpoint_path = CHECKPOINT_DIR / f"unetr_11ch_fold{fold_num+1}.pth"
    start_epoch = 0
...
def main():
    print("="*70)
    print("🛰️  Обучение UNETR на 11 каналах (B2-B9, B8A, B11, B12)")
    print("="*70)
    print(f"\n📂 Датасет: {BASE_DIR}")
